...
PRIV_TIMER_CHECK: //check if the interrupt came from the timer
    			CMP R5, #MPCORE_PRIV_TIMER_IRQ
				BNE KEYS_CHECK //if not timer, keys
				BL PRIV_TIMER_ISR
				B EXIT_IRQ
KEYS_CHECK: //check if interrupt from keys
    			CMP R5, #KEYS_IRQ
				BNE UNKNOWN_IRQ //if not timer or keys, unknown
				BL KEY_ISR
				B EXIT_IRQ
UNKNOWN_IRQ: 
...
SWEEP:		LDR		R0, =LEDR_DIRECTION	// put shifting direction into R2
				...
				LDR		R3, [R1] //R3 is the LEDR pattern which we store back to R1 (LEDs) at the end
				
				PUSH {R8, R9, R10} //this is a subroutine, can only use R0-3, therefore use 8-10 temporarily
				
				//pull first and second half of LEDS
				AND R8, R3, #0x3E0 //left half
				AND R9, R3, #0x01F //right half
				
				CMP R2, #0 //0 means shifting to center, 1 means shifting out
				BEQ TOCENTRE
				B TOOUTSIDE
//sweeping to the centre?
TOCENTRE:		//the left half gets shifted right 1, right half gets shifted left 1
				LSR R8, R8, #1
				LSL R9, R9, #1
				MOV R3, #0
				EOR R3, R8, R9 //store both halves back into R3
				CMP R3, #0x030 //edge case: LEDs in middle are both lit up? 
				BNE DONE_SWEEP //continue if not
C_O:			MOV		R2, #1					// change direction to outside
				B DONE_SWEEP

TOOUTSIDE:		LSL R8, R8, #1
				LSR R9, R9, #1
				EOR R3, R8, R9
				LDR R10, =0x201
				CMP R3, R10 //edge case: both LEDR0 and 9 lit up 
				BNE DONE_SWEEP //continue if not edge case
O_C:			MOV		R2, #0					// change direction to centre
				B DONE_SWEEP		//to centre if not

DONE_SWEEP:
				POP {R8, R9, R10} //registers return to original values
...
CHK_KEY3: //check if the key was pressed
				TST R1, #0b1000 //tst: compares r1 to the value if key 3 is pressed by using bitwise AND
				BEQ END_KEY_ISR //if it hasn't been pressed, do nothing

START_STOP: //runs if key has been pressed
				LDR		R0, =MPCORE_PRIV_TIMER		// timer base address
				//enable bit - starts and stops the timer
				LDR		R1, [R0, #0x8]			// read timer control register, holds I, A, E-> R0=E				
				CMP R1, #0b111 // if I, A, E = 111
				//toggle enable 
				MOVEQ R1, #0b110 //disable E
				MOVNE R1, #0b111 //if its disabled, enable it
				STR R1, [R0, #8] //store iae back into control reg
...
CONFIG_PRIV_TIMER:
			...
			LDR R1, =49999999 //for 0.25 seconds, 20M / 4 cycles
			STR R1, [R0] //store 50M into timer
			MOV R1, #0b111
			STR R1, [R0, #8] //setting I=1, A=1, E=1	
			MOV PC, LR 					// return

/* Configure the KEYS to generate an interrupt */
CONFIG_KEYS:
			LDR R0, =0xFF200050 		// KEYs base address
			MOV R1, #0x8 //key 3 is 0b1000 or 8 in hex
			STR R1, [R0, #8] //enable interrupt for key3
			MOV PC, LR 					// return
